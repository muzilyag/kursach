...
export interface IUserCreate extends Omit<
  IUser,
  'user_id' | 'user_registration_date' | 'active_subscription'
> {
...
export const ApiService = {
  ...
  async getContent(
    params: any
  ): Promise<{ items: IContent[]; total: number; page: number; pages: number }> {
    const query = this.buildQuery(params)
  ...
  async getTagsDirect(
    params: any = {}
  ): Promise<{ items: ITag[]; total: number; page: number; pages: number }> {
    const query = this.buildQuery(params)
  ...
  async getCopyrightHoldersDirect(
    params: any = {}
  ): Promise<{ items: ICopyrightHolder[]; total: number; page: number; pages: number }> {
    const query = this.buildQuery(params)
    return this.request(`${Config.api.copyrightHoldersDirect}?${query}`)
  },

  async createCopyrightHolder(
    holder: Omit<ICopyrightHolder, 'copyright_holder_id'>
  ): Promise<ICopyrightHolder> {
  ...
  async updateCopyrightHolder(
    id: number,
    holder: Partial<ICopyrightHolder>
  ): Promise<ICopyrightHolder> {
  ...
  async exportSeasonalityReport(year: number, format: 'csv' | 'pdf'): Promise<Blob> {
    return this.requestBlob(
      `${Config.api.reports}/seasonality?year=${year}&export=true&format=${format}`
    )
  },
  ...
  async exportRevenueReport(
    ...
    return this.requestBlob(
      `${Config.api.reports}/revenue?start_date=${startDate}&end_date=${endDate}&export=true&format=${format}`
    )
  },
