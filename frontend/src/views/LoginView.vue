    ...
    <div class="card shadow-lg border-0 p-4" style="max-width: 400px; width: 100%;">
      ...
      <form @submit.prevent="handleLogin">
        <div class="mb-3">
          ...
          <input v-model="form.identifier" type="text" class="form-control shadow-none" required placeholder="example@mail.ru">
        </div>
        <div class="mb-4">
          <label class="form-label small fw-bold text-uppercase">Пароль</label>
          <input v-model="form.password" type="password" class="form-control shadow-none" required placeholder="••••••••">
        </div>
      ...
      <div class="text-center mt-4">
        <p class="small text-muted mb-0">
          Нет аккаунта? 
          <router-link to="/register" class="text-decoration-none fw-bold">Зарегистрироваться</router-link>
        </p>
      </div>
    </div>
  </div>
