...
class SubscriptionRepository:
            ...
            items.append(
                {
                    ...
                    "status": "Активна"
                    if r["subscribe_finish"] >= today
                    else "Истекла",
                    "user": {"user_id": r["user_id"], "user_name": r["user_name"]},
                    "subscribe_type": {
                        "subscribe_type_id": r["subscribe_type_id"],
                        "subscribe_type_name": r["subscribe_type_name"],
                    },
                }
            )
    ...
    async def get_subscription_types(self):
        query = text(
            "SELECT subscribe_type_id as value, subscribe_type_name as label, subscribe_type_cost as cost FROM subscribe_type ORDER BY subscribe_type_id"
        )
