    ...
    <div class="card shadow-sm border-0 mb-4">
      <div class="card-body">
        <div class="row g-3 align-items-end">
          ...
          <template v-if="selectedReport === 'seasonality'">
            <div class="col-md-2">
              <label class="form-label small fw-bold text-secondary text-uppercase">С месяца</label>
              <input v-model="startMonth" type="month" :max="endMonth" class="form-control shadow-none" />
            </div>
            <div class="col-md-2">
              <label class="form-label small fw-bold text-secondary text-uppercase">По месяц</label>
              <input v-model="endMonth" type="month" :min="startMonth" class="form-control shadow-none" />
            </div>
          </template>
          ...
          <template v-if="selectedReport === 'revenue'">
            <div class="col-md-2">
              ...
              <input v-model="startDate" type="date" :max="endDate" class="form-control shadow-none" />
            </div>
            <div class="col-md-2">
              <label class="form-label small fw-bold text-secondary text-uppercase">Конец</label>
              <input v-model="endDate" type="date" :min="startDate" class="form-control shadow-none" />
            </div>
          </template>

          <div class="col-md-auto ms-auto d-flex gap-2">
            <button class="btn btn-primary px-4 shadow-sm" @click="generate" :disabled="loading || isInvalidDates">
            ...
            <button
              ...
              :disabled="!reportData.length || downloadingFormat === 'csv' || isInvalidDates"
            >
              <span
            ...
            <button
              ...
              :disabled="!reportData.length || downloadingFormat === 'pdf' || isInvalidDates"
            >
              <span
        ...
        <div v-if="isInvalidDates" class="text-danger small mt-2">
...
const generate = async () => {
  ...
  try {
    ...
    if (selectedReport.value === 'seasonality') {
      response = await ApiService.getSeasonalityReport(startMonth.value || '', endMonth.value || '')
    } else if (selectedReport.value === 'activity') {
...
const downloadReport = async (format: 'csv' | 'pdf') => {
  ...
  try {
    let blob: Blob
    if (selectedReport.value === 'seasonality') {
      blob = await ApiService.exportSeasonalityReport(startMonth.value || '', endMonth.value || '', format)
    } else if (selectedReport.value === 'activity') {
