...
const totalPaymentAmount = computed(() => {
  ...
  return stats.value.breakdown.payment_methods.reduce(
...
const totalTariffAmount = computed(() => {
  ...
  return stats.value.breakdown.revenue_by_tariffs.reduce(
    (acc: number, item: any) => acc + parseFloat(item.amount),
    0
  )
})
...
const totalSubsCount = computed(() => {
  ...
  return (
    stats.value.breakdown.subscriptions_status.active +
    stats.value.breakdown.subscriptions_status.expired
  )
})
    ...
    <div v-else-if="stats">
          ...
          >
            ...
            <div v-for="item in stats.breakdown.content_types" :key="item.type" class="mb-3">
              <div class="d-flex justify-content-between align-items-center mb-1">
                ...
                <span class="fw-bold small" style="color: var(--text-darker)"
                  >{{ item.count }} шт. ({{ getContentPercentage(item.count) }}%)</span
                >
              </div>
              <div
                class="progress"
                style="height: 10px; background-color: var(--light-bg); border-radius: 5px"
              >
                <div
                  class="progress-bar"
                  role="progressbar"
                  :style="{
                    width: getContentPercentage(item.count) + '%',
            ...
            <div v-for="item in stats.breakdown.payment_methods" :key="item.method" class="mb-3">
              <div class="d-flex justify-content-between align-items-center mb-1">
                ...
                <span class="fw-bold small" style="color: var(--text-darker)"
                  >{{ parseFloat(item.amount).toLocaleString() }} ₽ ({{
                    getPaymentPercentage(item.amount)
                  }}%)</span
                >
              </div>
              <div
              ...
              >
                <div
                  ...
                  role="progressbar"
                  :style="{ width: getPaymentPercentage(item.amount) + '%' }"
                ></div>
              </div>
            </div>
          </div>
      ...
      <div class="row g-4 mb-4">
              ...
              <div>
                <div class="d-flex justify-content-between align-items-center mb-1">
                  ...
                  <span class="fw-bold small text-success"
                    >{{ stats.breakdown.subscriptions_status.active }} шт. ({{
                      getSubPercentage(stats.breakdown.subscriptions_status.active)
                    }}%)</span
                  >
                </div>
                <div
                ...
                >
                  <div
                    ...
                    role="progressbar"
                    :style="{
                      width: getSubPercentage(stats.breakdown.subscriptions_status.active) + '%'
                    }"
                  ></div>
                </div>
              </div>
              <div>
                <div class="d-flex justify-content-between align-items-center mb-1">
                  ...
                  <span class="fw-bold small text-danger"
                    >{{ stats.breakdown.subscriptions_status.expired }} шт. ({{
                      getSubPercentage(stats.breakdown.subscriptions_status.expired)
                    }}%)</span
                  >
                </div>
                <div
                ...
                >
                  <div
                    ...
                    role="progressbar"
                    :style="{
                      width: getSubPercentage(stats.breakdown.subscriptions_status.expired) + '%'
                    }"
                  ></div>
                </div>
              </div>
        ...
        <div class="col-12 col-lg-6">
          ...
          >
            ...
            <div
              v-for="item in stats.breakdown.revenue_by_tariffs"
              :key="item.tariff_name"
              class="mb-3"
            >
              <div class="d-flex justify-content-between align-items-center mb-1">
                <span class="fw-bold text-muted small"
                  >{{ item.tariff_name }} ({{ item.count }} шт.)</span
                >
                <span class="fw-bold small" style="color: var(--text-darker)"
                  >{{ parseFloat(item.amount).toLocaleString() }} ₽ ({{
                    getTariffPercentage(item.amount)
                  }}%)</span
                >
              </div>
              <div
              ...
              >
                <div
                  ...
                  role="progressbar"
                  :style="{ width: getTariffPercentage(item.amount) + '%' }"
                ></div>
              </div>
            </div>
          </div>
      ...
      <div class="row g-4">
        <div class="col-12">
          ...
          >
            <div class="d-flex justify-content-between align-items-center mb-4">
              <h5 class="fw-bold m-0" style="color: var(--text-darker)">
                Динамика регистраций за неделю
              </h5>
              <div v-if="stats.breakdown.registrations_dynamics" class="text-end">
                <span class="fs-4 fw-bold text-primary"
                  >+{{ stats.breakdown.registrations_dynamics.total_new_this_week }}</span
                >
                <span class="small text-muted ms-2">новых пользователей</span>
                <span
                  v-if="stats.breakdown.registrations_dynamics.growth_percentage !== undefined"
                  :class="[
                    'small ms-2 fw-bold',
                    stats.breakdown.registrations_dynamics.growth_percentage >= 0
                      ? 'text-success'
                      : 'text-danger'
                  ]"
                >
                  ({{ stats.breakdown.registrations_dynamics.growth_percentage >= 0 ? '+' : ''
                  }}{{ stats.breakdown.registrations_dynamics.growth_percentage }}%)
                </span>
              </div>
            </div>
            <div
              v-if="stats.breakdown.registrations_dynamics?.daily"
              class="d-flex align-items-end justify-content-between pt-3"
              style="height: 150px"
            >
              <div
                v-for="day in stats.breakdown.registrations_dynamics.daily"
                :key="day.date"
                class="d-flex flex-column align-items-center flex-grow-1 mx-1"
              >
                <span class="small fw-bold mb-2" style="color: var(--text-darker)">{{
                  day.count
                }}</span>
                <div
                  class="w-100 rounded-top"
                  :style="{
                    height: (day.count / maxRegistrationCount) * 100 + 'px',
