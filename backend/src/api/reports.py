    ...
    if export:
        if format == "pdf":
            return create_pdf_response(
                data, f"seasonality_{year}", title=f"Отчёт по сезонности за {year} год"
            )
...
async def get_activity_report(
    ...
    if export:
        if format == "pdf":
            return create_pdf_response(
                data, "activity_report", title="Отчёт по активности пользователей"
            )
...
async def get_revenue_report(
    ...
    if export:
        if format == "pdf":
            ...
            return create_pdf_response(
                data, f"revenue_{start_date}_{end_date}", title=title
            )
